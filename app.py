...
def display_chat():
    ...
    for message in st.session_state["messages"]:
        ...
        # Check for source in the content
        if "(Source: Vector Store)" in content:
            style = "background-color:#E8E8E8;"
        elif "(Source: Wikipedia)" in content:
            style = "background-color:#F0F8FF;"  # Light blue for Wikipedia
        else:
            style = "background-color:#FFD700;"  # Yellow for unknown
...
if submit_button and user_input:
    ...
    for event in graph.stream({"question": user_input}):
        for key, value in event.items():
            # Handle the output and source identification
            if "documents" in value:
                response = value["documents"][0].page_content
                source = key  
            elif "generation" in value:
            ...
            # Add assistant response with source to the conversation
            st.session_state['messages'].append({
                "type": "assistant",
                "content": f"{response} (Source: {source})"
            })
